...
CTimeSpan::CTimeSpan(_In_ int nDays, _In_ int nHours, _In_ int nMinutes, _In_ int nSeconds, _In_opt_ int nMilliSeconds)
    : CBaseMemObj()
{
...
HRESULT CTimeSpan::SetFrom(_In_ double nValue, _In_ LONGLONG nTickMultiplicator)
{
    ...
    if (nClass == _FPCLASS_SNAN || nClass == _FPCLASS_QNAN || nClass == _FPCLASS_NINF || nClass == _FPCLASS_PINF)
    {
        return E_INVALIDARG;
    }
...
HRESULT CTimeSpan::Parse(_In_z_ LPCSTR szSrcA, _In_ SIZE_T nLength, _Out_opt_ LPSTR *lpszEndStringA)
{
    ...
    // get days count
    szBackupA = szSrcA;
    if (myStrTol(nDays, szSrcA, nLength) == FALSE || nDays < 0)
    {
    ...
    if (nLength > 0 && *szSrcA == '.')
    {
        ...
        nLength--;
        // and parse hours
        if (myStrTol(nHours, szSrcA, nLength) == FALSE || nHours < 0 || nHours > 23)
        {
            hRes = MX_E_InvalidData;
            goto parse_err;
        }
    ...
    nLength--;
    // get minutes count
    if (myStrTol(nMinutes, szSrcA, nLength) == FALSE || nMinutes < 0 || nMinutes > 59)
    {
        hRes = MX_E_InvalidData;
        goto parse_err;
    }
    ...
    nLength--;
    // get seconds count
    if (myStrTol(nSeconds, szSrcA, nLength) == FALSE || nSeconds < 0 || nSeconds > 59)
    {
        hRes = MX_E_InvalidData;
        goto parse_err;
    }
    ...
    if (nLength > 0 && *szSrcA == L'.')
    {
        ...
        nLength--;
        // get ticks count
        if (myStrTol(nTicksX, szSrcA, nLength) == FALSE || nTicksX < 0 || nTicksX > 9999999)
        {
            hRes = MX_E_InvalidData;
            goto parse_err;
        }
...
HRESULT CTimeSpan::Parse(_In_z_ LPCWSTR szSrcW, _In_ SIZE_T nLength, _Out_opt_ LPWSTR *lpszEndStringW)
{
    ...
    // get days count
    szBackupW = szSrcW;
    if (myWcsTol(nDays, szSrcW, nLength) == FALSE || nDays < 0)
    {
        hRes = MX_E_InvalidData;
        goto parse_err;
    }
    ...
    if (nLength > 0 && *szSrcW == L'.')
    {
        ...
        nLength--;
        // and parse hours
        if (myWcsTol(nHours, szSrcW, nLength) == FALSE || nHours < 0 || nHours > 23)
        {
            hRes = MX_E_InvalidData;
            goto parse_err;
        }
    ...
    nLength--;
    // get minutes count
    if (myWcsTol(nMinutes, szSrcW, nLength) == FALSE || nMinutes < 0 || nMinutes > 59)
    {
        hRes = MX_E_InvalidData;
        goto parse_err;
    }
    ...
    nLength--;
    // get seconds count
    if (myWcsTol(nSeconds, szSrcW, nLength) == FALSE || nSeconds < 0 || nSeconds > 59)
    {
        hRes = MX_E_InvalidData;
        goto parse_err;
    }
    ...
    if (nLength > 0 && *szSrcW == L'.')
    {
        ...
        nLength--;
        // get ticks count
        if (myWcsTol(nTicksX, szSrcW, nLength) == FALSE || nTicksX < 0 || nTicksX > 9999999)
        {
            hRes = MX_E_InvalidData;
            goto parse_err;
        }
                               ...
                               _Out_opt_ LPBOOL lpbOverflow)
{
    ...
    if (nHours < 0 || nHours > 23 || nMinutes < 0 || nMinutes > 59 || nSeconds < 0 || nSeconds > 59 ||
        nMilliSeconds < 0 || nMilliSeconds > 999)
    {
        if (lpbOverflow != NULL)
        {
            *lpbOverflow = TRUE;
