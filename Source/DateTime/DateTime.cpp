...
HRESULT CDateTime::SetFromSystemTime(_In_ const SYSTEMTIME &sSrcSysTime)
{
    ...
    hRes = SetDateTime((int)sSrcSysTime.wYear, (int)sSrcSysTime.wMonth, (int)sSrcSysTime.wDay, (int)sSrcSysTime.wHour,
                       (int)sSrcSysTime.wMinute, (int)sSrcSysTime.wSecond, (int)sSrcSysTime.wMilliseconds);
    if (SUCCEEDED(hRes))
    {
        nGmtOffset = 0;
    }
...
HRESULT CDateTime::Add(_In_ LONGLONG nCount, _In_ CDateTime::eUnits nUnits)
{
    ...
    if (SUCCEEDED(hRes) && (cNewTicks.GetTicks() < 0 || cNewTicks.GetTicks() > X_MAX_VALUE_TICKS))
    {
        hRes = MX_E_ArithmeticOverflow;
    }
    if (SUCCEEDED(hRes))
