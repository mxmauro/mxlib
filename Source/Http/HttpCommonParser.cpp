...
VOID CHttpParser::Reset()
{
    ...
    sRequest.cUrl.Reset();
    sRequest.nBrowser = Http::eBrowser::Other;
    //----
...
HRESULT CHttpParser::ParseRequestLine(_In_z_ LPCSTR szLineA)
{
    ...
    for (nUriLength = 0; szUriStartA[nUriLength] == 9 || szUriStartA[nUriLength] == 12 ||
                         (szUriStartA[nUriLength] >= 33 && szUriStartA[nUriLength] <= 126);
         nUriLength++)
    {
        ;
    }
    ...
    if ((nUriLength >= 7 && StrNCompareA(szUriStartA, "http://", 7) == 0) ||
        (nUriLength >= 8 && StrNCompareA(szUriStartA, "https://", 8) == 0) || szUriStartA[0] == '/')
    {
    ...
    if (szLineA[0] != 'H' || szLineA[1] != 'T' || szLineA[2] != 'T' || szLineA[3] != 'P' || szLineA[4] != '/' ||
        szLineA[5] != '1' || szLineA[6] != '.' || (szLineA[7] != '0' && szLineA[7] != '1'))
    {
        return MX_E_InvalidData;
    }
...
HRESULT CHttpParser::ParseStatusLine(_In_z_ LPCSTR szLineA)
{
    // check http version
    if (szLineA[0] != 'H' || szLineA[1] != 'T' || szLineA[2] != 'T' || szLineA[3] != 'P' || szLineA[4] != '/' ||
        szLineA[5] != '1' || szLineA[6] != '.' || (szLineA[7] != '0' && szLineA[7] != '1') || szLineA[8] != ' ' ||
        szLineA[9] < '1' || szLineA[9] > '5' || szLineA[10] < '0' || szLineA[10] > '9' || szLineA[11] < '0' ||
        szLineA[11] > '9' || szLineA[12] != ' ')
    {
        return MX_E_InvalidData;
    }
    ...
    sResponse.nStatusCode = (LONG)(szLineA[9] - '0') * 100 + (LONG)(szLineA[10] - '0') * 10 + (LONG)(szLineA[11] - '0');
