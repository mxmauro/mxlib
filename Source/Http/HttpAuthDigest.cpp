    ...
    if (SUCCEEDED(hRes))
    {
        ...
        if (SUCCEEDED(hRes))
        {
            if (cStrDestA.ConcatN(", uri=\"", 7) == FALSE ||
                cStrDestA.ConcatN(szUriPathA, StrLenA(szUriPathA)) == FALSE || cStrDestA.ConcatN("\"", 1) == FALSE)
            {
                hRes = E_OUTOFMEMORY;
            }
        }
...
static BOOL ConvertToHex(_Out_ MX::CStringA &cStrA, _In_ LPCVOID lpData, _In_ SIZE_T nDataLen)
{
    ...
    while (s < sEnd)
    {
        if (cStrA.ConcatN(&szHexaA[(*s) >> 4], 1) == FALSE || cStrA.ConcatN(&szHexaA[(*s) & 0x0F], 1) == FALSE)
        {
            return FALSE;
        }
        s++;
