...
HRESULT CHttpCookie::ToString(_Inout_ CStringA &cStrDestA, _In_ BOOL bAddAttributes)
{
    ...
    lpDt = NULL;
    if (cStrDestA.Copy((LPCSTR)cStrNameA) == FALSE || cStrDestA.Concat("=") == FALSE)
    {
        return E_OUTOFMEMORY;
    }
    ...
    if (bAddAttributes != FALSE)
    {
        ...
        {
            if (cStrDestA.Concat("; Path=") == FALSE || cStrDestA.Concat((LPSTR)cStrPathA) == FALSE)
            {
                return E_OUTOFMEMORY;
            }
        }
        ...
        {
            if (cStrDestA.Concat("; Domain=") == FALSE || cStrDestA.Concat((LPSTR)cStrDomainA) == FALSE)
            {
                return E_OUTOFMEMORY;
            }
        }
...
CHttpCookieArray::CHttpCookieArray(_In_ const CHttpCookieArray &cSrc) throw(...)
    : TArrayListWithRelease<CHttpCookie *>()
{
    operator=(cSrc);
    return;
}
...
HRESULT CHttpCookieArray::RemoveExpiredAndInvalid(_In_opt_ const CDateTime *lpDate)
{
    ...
    while (nCount > 0)
    {
        ...
        if (lpCookie->GetName()[0] == 0 || lpCookie->GetValue()[0] == 0 || lpCookie->HasExpired(lpDate) == S_OK)
        {
