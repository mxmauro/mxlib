...
#pragma warning(suppress : 6387)
    lpDef = (HTMLENTITY_DEF *)bsearch_s(&chW, aHtmlEntities, MX_ARRAYLEN(aHtmlEntities), sizeof(HTMLENTITY_DEF), &HtmlEntities_Compare,
                                        NULL);
    ...
    while (szStrA < szEndA)
    {
        ...
        if (nChars > 0 && chW[1] == 0)
        {
            ...
            if (szEntityA != NULL)
            {
                ...
                // add entity
                if (cStrA.ConcatN("&", 1) == FALSE || cStrA.Concat(szEntityA) == FALSE || cStrA.ConcatN(";", 1) == FALSE)
                {
                    return E_OUTOFMEMORY;
                }
...
#pragma warning(suppress : 6054)
                cStrW.ConcatN(szTempBufW, i) == FALSE || cStrW.ConcatN(";", 1) == FALSE)
            {
                return E_OUTOFMEMORY;
            }
