...
HRESULT CProxy::SetCredentials(_In_opt_z_ LPCWSTR szUserNameW, _In_opt_z_ LPCWSTR szPasswordW)
{
    ...
    if (szUserNameW != NULL && *szUserNameW != 0 && cStrTempUserNameW.Copy(szUserNameW) == FALSE)
    {
        return E_OUTOFMEMORY;
    }
    if (szPasswordW != NULL && *szPasswordW != 0 && cStrTempUserPasswordW.Copy(szPasswordW) == FALSE)
    {
        return E_OUTOFMEMORY;
    }
...
VOID CIEProxyResolver::RetrieveProxySettings()
{
    ...
    if (fnWinHttpGetIEProxyConfigForCurrentUser(&sIeProxy) != FALSE)
    {
        ...
        {
            if (cStrNewAutoConfigUrlW.Copy(sIeProxy.lpszAutoConfigUrl) == FALSE)
            {
                cStrNewAutoConfigUrlW.Empty();
            }
        }
    }
                                       ...
                                       _In_opt_z_ LPCWSTR szAutoConfigUrlW, _Out_ LPWSTR *lpwszProxyW)
{
    ...
    *lpwszProxyW = NULL;
    ...
    if (*szUrlW != 0)
    {
        ...
        // First call with no autologon.  Autologon prevents the session (in proc) or autoproxy service (out of proc)
        // from caching the proxy script.  This causes repetitive network traffic, so it is best not to do autologon
        // unless it is required according to the result of WinHttpGetProxyForUrl. See:
