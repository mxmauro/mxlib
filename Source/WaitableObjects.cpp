...
static int __cdecl InitializeKernel32Apis()
{
    ...
    if (hDll != NULL)
    {
        ...
        if (fnInitializeSRWLock == NULL || fnReleaseSRWLockExclusive == NULL || fnReleaseSRWLockShared == NULL ||
            ...
            fnTryAcquireSRWLockExclusive == NULL || fnTryAcquireSRWLockShared == NULL)
        {
            ...
            fnReleaseSRWLockExclusive = NULL;
            fnReleaseSRWLockShared = NULL;
            fnAcquireSRWLockExclusive = NULL;
            fnAcquireSRWLockShared = NULL;
            fnTryAcquireSRWLockExclusive = NULL;
            fnTryAcquireSRWLockShared = NULL;
        }
    }
    ...
    *lphRootDir = __InterlockedReadPointer(&hRootDirHandle);
    ...
    if (sOviW.dwMajorVersion >= 6)
    {
        ...
        if (sOviW.dwMajorVersion > 6 || sOviW.dwMinorVersion >= 2)
        {
            ...
            nNtStatus = ::MxNtQueryInformationToken(
                hProcessToken, (TOKEN_INFORMATION_CLASS)42 /*TokenPrivateNameSpace*/, &dwUsesPrivateNamespace,
                (ULONG)sizeof(dwUsesPrivateNamespace), &dwRetLength);
            if (!NT_SUCCESS(nNtStatus))
            {
                goto done;
            }
