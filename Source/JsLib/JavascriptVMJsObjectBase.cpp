...
CJsObjectBase *CJsObjectBase::FromObject(_In_ DukTape::duk_context *lpCtx, _In_ DukTape::duk_int_t nIndex)
{
    ...
    hRes = lpJVM->RunNativeProtectedAndGetError(0, 0,
                                                [&lpObj, nIndex](_In_ DukTape::duk_context *lpCtx) -> VOID
                                                {
                                                    DukTape::duk_get_prop_string(lpCtx, nIndex,
                                                                                 "\xff"
                                                                                 "\xff"
                                                                                 "data");
                                                    if (DukTape::duk_is_undefined(lpCtx, -1) == 0)
                                                    {
                                                        lpObj = reinterpret_cast<CJsObjectBase *>(
...
int CJsObjectBase::OnProxyHasNamedProperty(_In_ DukTape::duk_context *lpCtx, _In_z_ LPCSTR szPropNameA)
{
    UNREFERENCED_PARAMETER(lpCtx);
    UNREFERENCED_PARAMETER(szPropNameA);
    return -1; // pass original
}

int CJsObjectBase::OnProxyHasIndexedProperty(_In_ DukTape::duk_context *lpCtx, _In_ int nIndex)
{
    UNREFERENCED_PARAMETER(lpCtx);
...
int CJsObjectBase::OnProxyGetNamedProperty(_In_ DukTape::duk_context *lpCtx, _In_z_ LPCSTR szPropNameA)
{
    UNREFERENCED_PARAMETER(lpCtx);
    UNREFERENCED_PARAMETER(szPropNameA);
...
int CJsObjectBase::OnProxyGetIndexedProperty(_In_ DukTape::duk_context *lpCtx, _In_ int nIndex)
{
    UNREFERENCED_PARAMETER(lpCtx);
    UNREFERENCED_PARAMETER(nIndex);
    return 0; // pass original
...
int CJsObjectBase::OnProxySetNamedProperty(_In_ DukTape::duk_context *lpCtx, _In_z_ LPCSTR szPropNameA,
                                           _In_ DukTape::duk_idx_t nValueIndex)
{
    UNREFERENCED_PARAMETER(lpCtx);
    UNREFERENCED_PARAMETER(szPropNameA);
    UNREFERENCED_PARAMETER(nValueIndex);
    return 0; // set original
}

int CJsObjectBase::OnProxySetIndexedProperty(_In_ DukTape::duk_context *lpCtx, _In_ int nIndex,
                                             _In_ DukTape::duk_idx_t nValueIndex)
{
    UNREFERENCED_PARAMETER(lpCtx);
    UNREFERENCED_PARAMETER(nIndex);
...
int CJsObjectBase::OnProxyDeleteNamedProperty(_In_ DukTape::duk_context *lpCtx, _In_z_ LPCSTR szPropNameA)
{
    UNREFERENCED_PARAMETER(lpCtx);
    UNREFERENCED_PARAMETER(szPropNameA);
...
int CJsObjectBase::OnProxyDeleteIndexedProperty(_In_ DukTape::duk_context *lpCtx, _In_ int nIndex)
{
    UNREFERENCED_PARAMETER(lpCtx);
    UNREFERENCED_PARAMETER(nIndex);
    return 1; // delete original
...
LPCSTR CJsObjectBase::OnProxyGetPropertyName(_In_ DukTape::duk_context *lpCtx, _In_ int nIndex)
{
    UNREFERENCED_PARAMETER(lpCtx);
    UNREFERENCED_PARAMETER(nIndex);
    return NULL;
                                       ...
                                       _In_ lpfnRegisterUnregisterCallback fnUnregisterCallback)
{
    ...
    hRes = lpJVM->RunNativeProtectedAndGetError(0, 0,
                                                [=](_In_ DukTape::duk_context *lpCtx) -> VOID
                                                {
                                                    ...
                                                    // push the constructor object, all of our object will have a
                                                    // constructor
                                                    DukTape::duk_push_c_function(
                                                        lpCtx, &CJsObjectBase::_ConstructorHelper, MX_JS_VARARGS);

                                                    // but just register callback function if the object is really
                                                    // creatable
                                                    ...
                                                    // set internal property to check if we have to create a proxy
                                                    // object too
                                      ...
                                      _In_ lpfnRegisterUnregisterCallback fnUnregisterCallback)
{
    ...
    lpJVM->RunNativeProtectedAndGetError(0, 0,
                                         [fnUnregisterCallback](_In_ DukTape::duk_context *lpCtx) -> VOID
                                         {
    ...
    lpJVM->RunNativeProtectedAndGetError(0, 0,
                                         [szObjectNameA](_In_ DukTape::duk_context *lpCtx) -> VOID
                                         {
                                             DukTape::duk_push_global_object(lpCtx);
...
DukTape::duk_ret_t CJsObjectBase::_ConstructorHelper(_In_ DukTape::duk_context *lpCtx)
{
    ...
    DukTape::duk_get_prop_string(lpCtx, -1,
                                 "\xff"
                                 "\xff"
                                 "createObject");
    ...
    // create object
    lpNewObj = fnCreateObject();
    if (lpNewObj != NULL)
    {
        hRes = lpNewObj->PushThis(lpCtx);
...
HRESULT CJsObjectBase::_PushThisHelper(_In_ DukTape::duk_context *lpCtx, _In_z_ LPCSTR szObjectNameA)
{
    CJavascriptVM *lpJVM = CJavascriptVM::FromContext(lpCtx);
    HRESULT hRes;
    ...
    hRes = lpJVM->RunNativeProtectedAndGetError(
        0, 1,
        [this, szObjectNameA](_In_ DukTape::duk_context *lpCtx) -> VOID
        {
            MAP_ENTRY *lpEntries;
            BOOL bCreateProxy;
            ...
            DukTape::duk_get_prop_string(lpCtx, -1,
                                         ...
                                         "useProxy");
            bCreateProxy =
                (DukTape::duk_is_boolean(lpCtx, -1) != 0 && DukTape::duk_get_boolean(lpCtx, -1) != 0) ? TRUE : FALSE;
...
DukTape::duk_ret_t CJsObjectBase::_FinalReleaseHelper(_In_ DukTape::duk_context *lpCtx)
{
    ...
    DukTape::duk_get_prop_string(lpCtx, 0,
                                 "\xff"
                                 "\xff"
                                 "released");
    if (!DukTape::duk_to_boolean(lpCtx, -1))
    {
        ...
        DukTape::duk_put_prop_string(lpCtx, 0,
                                     "\xff"
                                     "\xff"
                                     "released");
    }
...
DukTape::duk_ret_t CJsObjectBase::_ProxyOwnKeysHelper(_In_ DukTape::duk_context *lpCtx)
{
    ...
    for (nIndex = 0;; nIndex++)
    {
        LPCSTR szPropNameA = lpObj->OnProxyGetPropertyName(lpCtx, nIndex);
