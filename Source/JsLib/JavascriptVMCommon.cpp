                                ...
                                _In_ int nArgsCount)
{
    ...
    hRes = lpJVM->RunNativeProtectedAndGetError(
        ...
        [=](_In_ DukTape::duk_context *lpCtx) -> VOID
        {
            ...
            // store callback
            p = DukTape::duk_push_fixed_buffer(
                lpCtx, MX::CJavascriptVM::OnNativeFunctionCallback::serialization_buffer_size());
                          ...
                          _In_ MX::CJavascriptVM::OnSetPropertyCallback cSetValueCallback)
{
    ...
    hRes = lpJVM->RunNativeProtectedAndGetError(
        (bInitialValueOnStack != FALSE) ? 1 : 0, 0,
        [&](_In_ DukTape::duk_context *lpCtx) -> VOID
        {
            ...
            if (cGetValueCallback)
            {
                ...
                // store callback
                p = DukTape::duk_push_fixed_buffer(
                    lpCtx, MX::CJavascriptVM::OnGetPropertyCallback::serialization_buffer_size());
            ...
            if (cSetValueCallback)
            {
                ...
                // store callback
                p = DukTape::duk_push_fixed_buffer(
                    lpCtx, MX::CJavascriptVM::OnSetPropertyCallback::serialization_buffer_size());
        ...
        while (1)
        {
            ...
            while (*szObjectNameA == '[')
            {
                ...
                if (*szObjectNameA == '"' || *szObjectNameA == '\'')
                {
                    ...
                    if (nNameLen == 0 || szObjectNameA[nNameLen] != *(szObjectNameA - 1) ||
                        szObjectNameA[nNameLen + 1] != L']')
                    {
