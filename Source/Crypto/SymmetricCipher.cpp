                                       ...
                                       _In_opt_ LPCVOID lpInitVector)
{
    ...
    // initialize
    ERR_clear_error();
    if (EVP_EncryptInit(symcipher_data->cEncryptor.lpCipherCtx, symcipher_data->lpCipher, (const unsigned char *)lpKey,
                                       ...
                                       _In_opt_ LPCVOID lpInitVector)
{
    ...
    // initialize
    ERR_clear_error();
    if (EVP_DecryptInit(symcipher_data->cDecryptor.lpCipherCtx, symcipher_data->lpCipher, (const unsigned char *)lpKey,
                        (const unsigned char *)lpInitVector) <= 0)
    {
        hRes = Internals::OpenSSL::GetLastErrorCode(MX_E_InvalidData);
        symcipher_data->cDecryptor.Reset(TRUE);
