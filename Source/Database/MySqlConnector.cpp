...
typedef ULONG(__stdcall *lpfn_mysql_real_escape_string)(MYSQL *mysql, LPCSTR to, const LPCSTR from, ULONG length);
...
typedef int(__stdcall *lpfn_mysql_stmt_prepare)(MYSQL_STMT *stmt, const char *stmt_str, unsigned long length);
    ...
    if (mysql_data->lpResultSet != NULL)
    {
        ...
        if (nFieldsCount > 0)
        {
            ...
            if (mysql_data->lpStmt != NULL)
            {
                ...
                mysql_data->cOutputBindings.Attach((MYSQL_BIND *)MX_MALLOC(
                    nFieldsCount * (sizeof(MYSQL_BIND) + sizeof(ULONG) + sizeof(my_bool) + sizeof(my_bool))));
                if (!(mysql_data->cOutputBindings))
                {
                    goto err_nomem;
                }
                ::MxMemSet(mysql_data->cOutputBindings.Get(), 0,
                           nFieldsCount * (sizeof(MYSQL_BIND) + sizeof(ULONG) + sizeof(my_bool) + sizeof(my_bool)));
...
#define LOAD_API(x)                                                                                                    \
    fn_##x = (lpfn_##x)::GetProcAddress(_hDll, #x);                                                                    \
    if (fn_##x == NULL)                                                                                                \
        goto on_loaderror;
