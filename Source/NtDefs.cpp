...
extern "C"
{
                          ...
                          _In_opt_ LPSECURITY_ATTRIBUTES lpSecurityAttributes)
    {
        ...
        if (fnRtlDosPathNameToNtPathName_U == NULL)
        {
            ...
            fnRtlDosPathNameToNtPathName_U =
                (lpfnRtlDosPathNameToNtPathName_U)::MxGetProcedureAddress(DllBase, "RtlDosPathNameToNtPathName_U");
            if (fnRtlDosPathNameToNtPathName_U == NULL)
            {
    ...
    BOOL MxWriteMem(_In_ HANDLE hProcess, _In_ LPVOID lpDest, _In_ LPVOID lpSrc, _In_ SIZE_T nBytesCount)
    {
        ...
        return (NT_SUCCESS(nStatus) || (nStatus == STATUS_PARTIAL_COPY && nWritten == nBytesCount)) ? TRUE : FALSE;
    }
        ...
        if (lpFirstLink != NULL)
        {
            ...
            else
            {
                ...
                while (lpCurrLink != lpFirstLink)
                {
                    ...
                    if (sCurrEntry64.DllBase != 0 && (SIZE_T)(sCurrEntry64.BaseDllName.Length) == nModuleNameLen &&
