...
#define __MX_JS_DECLARE_COMMON(_cls, _name)                                                                            \
  public:                                                                                                              \
    ...
    {                                                                                                                  \
        return _RegisterHelper(lpCtx, _GetMapEntries(), bUseProxy, _name, fnCreateObject, &_cls::OnRegister,           \
                               &_cls::OnUnregister);                                                                   \
    };                                                                                                                 \
                                                                                                                       \
  public:                                                                                                              \
...
#define MX_JS_THROW_WINDOWS_ERROR(ctx, hr)                                                                             \
    MX::CJavascriptVM::ThrowWindowsError((ctx), (HRESULT)(hr), (LPCSTR)(__FILE__), (DukTape::duk_int_t)__LINE__)

#define MX_JS_THROW_ERROR(ctx, code, fmt, ...)                                                                         \
    DukTape::duk_error_raw((ctx), (DukTape::duk_errcode_t)(code), (LPCSTR)(__FILE__), (DukTape::duk_int_t)__LINE__,    \
                           (fmt), __VA_ARGS__)
...
class CJavascriptVM : public virtual CBaseMemObj, public CNonCopyableObj
{
  ...
  public:
    ...
    typedef Callback<int(_In_ DukTape::duk_context *lpCtx, _In_z_ LPCSTR szObjectNameA, _In_z_ LPCSTR szPropertyNameA,
        ...
        OnProxySetNamedPropertyCallback;
    typedef Callback<int(_In_ DukTape::duk_context *lpCtx, _In_z_ LPCSTR szObjectNameA, _In_ int nIndex,
                         _In_ DukTape::duk_idx_t nValueIndex)>
        OnProxySetIndexedPropertyCallback;

    // return 1 if delete must proceed, 0 to silently ignore, -1 to throw an error
    typedef Callback<int(_In_ DukTape::duk_context *lpCtx, _In_z_ LPCSTR szObjectNameA, _In_z_ LPCSTR szPropertyNameA)>
        OnProxyDeleteNamedPropertyCallback;
    typedef Callback<int(_In_ DukTape::duk_context *lpCtx, _In_z_ LPCSTR szObjectNameA, _In_ int nIndex)>
        OnProxyDeleteIndexedPropertyCallback;

    // return NULL/Empty String to end enumeration
    typedef Callback<LPCSTR(_In_ DukTape::duk_context *lpCtx, _In_z_ LPCSTR szObjectNameA, _In_ int nIndex)>
        OnProxyGetPropertyNameCallback;
