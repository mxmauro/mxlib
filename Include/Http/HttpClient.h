...
public:
    ...
    typedef Callback<HRESULT(_In_ CHttpClient *lpHttp, _In_z_ LPCWSTR szFileNameW, _In_opt_ PULONGLONG lpnContentSize,
                             ...
                             _Outptr_opt_result_maybenull_ CHttpBodyParserBase **lpBodyParser)>
        OnHeadersReceivedCallback;
    typedef Callback<VOID(_In_ CHttpClient *lpHttp)> OnDocumentCompletedCallback;

    typedef Callback<HRESULT(_In_ CHttpClient *lpHttp, _In_ CWebSocket *lpWebSocket, _In_z_ LPCSTR szProtocolA)>
        OnWebSocketHandshakeCompletedCallback;
...
public:
    ...
    template <class T> HRESULT AddRequestHeader(_Out_ T **lplpHeader = NULL)
    {
